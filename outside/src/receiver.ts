...
(async () => {
    ...
    for (const redis of connList) {
        ...
        redis.redis.on('error', () => {
            logger(`Connecting again for ${redis.address}`)
            redis.redis.connect()
        });
        (async () => {
            ...
            blockingConn.on('error', () => {
                logger(`BC Connecting again for ${redis.address}`)
                redis.redis.connect()
            })
            while (true) {
                try {
                    ...
                    if (typeof close === "boolean") { // close all sockets because client is gone
                        if (close) {
                            ...
                            let i = 0
                            for (const element of connList) {
                                element.redis.del(`receiver_chunks${i}`)
                                i++
                            }
                    ...
                    Object.entries(ids).forEach(async ([key, value]) => {
                                ...
                                if (pressure >= 9437184) {
                                    pq_reqsreps.add(() => {
                                        ...
                                        if (flatAss.size != 0) {
                                            ...
                                            logger("what database? " + redis.address + " then what list? " + j)
                                            redis.redis.rpush(`receiver_chunks${j}`, JSON.stringify(ass)).catch((e) => {
                                ...
                                fuckingTimeout = setTimeout(() => {
                                    ...
                                    pq_reqsreps.add(() => {
                                        ...
                                        if (flatAss.size != 0 && reqsReps.size == 0) {
                                            ...
                                            redis.redis.rpush(`receiver_chunks${j}`, JSON.stringify(Object.fromEntries(flatAss))).catch((e) => {
